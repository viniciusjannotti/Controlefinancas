...
export default function Dashboard() {
  ...
  const [historyMethods, setHistoryMethods] = useState<string[]>(PAYMENT_METHODS);
  ...
  const { availableYears, availableCategories, availableSubcategories, historyMontlyData } = React.useMemo(() => {
    ...
    sourceData.forEach(e => {
      ...
      let matchesCat = true;
      let matchesSub = true;
      const matchesMethod = historyMethods.includes(e.method);
      ...
      if (matchesCat && matchesSub && matchesMethod) {
  ...
  }, [allExpensesList, allEarningsList, historyTopic, historyCategory, historySubcategory, historyMethods, selectedYear]);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <p className="text-slate-500 dark:text-slate-400 animate-pulse">Carregando dados do dashboard...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-2">
        <h2 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Painel Geral</h2>
        <p className="text-slate-500 dark:text-slate-400 text-lg">Visão consolidada da saúde financeira da família.</p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        ...
        <Card className="col-span-3 cursor-pointer hover:shadow-lg hover:border-primary/20 transition-all duration-300 group" onClick={() => categoryData.length > 0 && setSelectedCategory(categoryData[0].name)}>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 capitalize bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md">
            ...
            {categoryData.length > 0 ? (
              <div className="w-full h-full relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-xs text-slate-400 dark:text-slate-500 font-medium group-hover:text-primary transition-colors">Clique para</span>
                  <span className="text-xs text-slate-400 dark:text-slate-500 font-bold group-hover:text-primary transition-colors">detalhes</span>
                </div>
              </div>
            ) : (
              <p className="text-slate-400 dark:text-slate-500 text-sm italic">Nenhum gasto registrado.</p>
            )}
          </CardContent>
        </Card>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
          <Card className="w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl scale-100 animate-in zoom-in-95 duration-300 flex flex-col">
            <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4 shrink-0">
              <div>
                ...
                <div className="flex items-center gap-4 mt-3">
                  <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-1 shadow-sm">
                    <button className="p-1 hover:bg-slate-200 rounded-lg text-slate-600 dark:text-slate-300" onClick={() => setCategoryMonth(subMonths(categoryMonth, 1))}>
                      <ChevronLeft className="w-4 h-4" />
                    </button>
                    <span className="text-sm font-semibold w-24 text-center capitalize text-slate-700 dark:text-slate-300">
                      {format(categoryMonth, 'MMM yyyy', { locale: ptBR })}
                    </span>
                    <button className="p-1 hover:bg-slate-200 rounded-lg text-slate-600 dark:text-slate-300" onClick={() => setCategoryMonth(addMonths(categoryMonth, 1))}>
                      <ChevronRight className="w-4 h-4" />
                    </button>
                  </div>
                  <p className="text-slate-500 dark:text-slate-400 text-sm hidden sm:block">Navegue pelas categorias para ver as subcategorias de cada uma.</p>
                </div>
              </div>
              <button
            ...
            <CardContent className="p-0 overflow-hidden flex-1 flex flex-col md:flex-row">
              <div className="w-full md:w-1/3 bg-slate-50 dark:bg-slate-800/50/50 border-r border-slate-100 dark:border-slate-800 overflow-x-auto md:overflow-y-auto flex flex-row md:flex-col p-4 gap-2 shrink-0 md:h-full">
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      selectedCategory === cat.name
                        ...
                        : "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-primary/30 hover:bg-slate-50"
                    )}
                  >
                    ...
                    <span className={cn("text-xs ml-4 shrink-0", selectedCategory === cat.name ? "text-primary-foreground/80" : "text-slate-400 dark:text-slate-500 group-hover:text-slate-600")}>
              ...
              <div className="w-full md:w-2/3 p-6 overflow-y-auto">
                <div className="flex items-center justify-between mb-8">
                  <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-200">{selectedCategory}</h3>
                ...
                <div className="grid lg:grid-cols-2 gap-8 items-start">
                  <div className="h-[250px] flex items-center justify-center bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-4">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
      ...
      <Card className="w-full">
        <CardHeader className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm">Analise a evolução dos gastos ou ganhos com filtros por categoria ao longo do ano.</p>
          </div>
          <div className="flex flex-wrap gap-3 items-center">
            <select
              className="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/20 cursor-pointer hover:border-slate-300 transition-colors"
              value={selectedYear}
            ...
            <select
              className="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/20 cursor-pointer hover:border-slate-300 transition-colors max-w-[200px]"
            ...
            {historyCategory !== 'Todas' && availableSubcategories.length > 1 && (
              <select
                className="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/20 cursor-pointer hover:border-slate-300 transition-colors animate-in fade-in zoom-in duration-200 max-w-[200px]"
            ...
            <div className="relative">
              <Button
                variant={showMethodFilter ? "secondary" : "outline"}
                className={cn(
                  "h-9 px-3 text-sm font-medium",
                  historyMethods.length < PAYMENT_METHODS.length && "border-primary text-primary"
                )}
                onClick={() => setShowMethodFilter(!showMethodFilter)}
              >
                <Filter className="w-4 h-4 mr-2" />
                Pagamento
                {historyMethods.length < PAYMENT_METHODS.length && (
              ...
              {showMethodFilter && (
                <>
                  <div className="fixed inset-0 z-10" onClick={() => setShowMethodFilter(false)} />
                  <div className="absolute right-0 top-full mt-2 z-20 w-56 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 p-3 animate-in fade-in zoom-in duration-200">
                    <div className="flex justify-between items-center mb-2 px-1">
                      <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Meio de Pagamento</span>
                      <button
                        className="text-[11px] text-primary font-medium hover:underline"
                        onClick={() => setHistoryMethods(
                          historyMethods.length === PAYMENT_METHODS.length ? [] : PAYMENT_METHODS
                        )}
                      >
                        {historyMethods.length === PAYMENT_METHODS.length ? "Limpar" : "Selecionar todos"}
                      </button>
                    </div>
                    <div className="space-y-1">
                      {PAYMENT_METHODS.map(method => (
                        <label
                          key={method}
                          className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-slate-50 cursor-pointer text-sm text-slate-700 dark:text-slate-300"
                        >
                          <input
                            type="checkbox"
                            className="rounded border-slate-300 text-primary focus:ring-primary/30"
                            checked={historyMethods.includes(method)}
                            onChange={() => setHistoryMethods(prev =>
        ...
        <CardContent>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50/50">
                  <th className="text-left py-4 px-6 text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest sticky left-0 bg-slate-50 dark:bg-slate-800/50/50 z-10 border-r border-slate-100 dark:border-slate-800 min-w-[160px]">
                    Item / Mês
                  </th>
                  {historyMontlyData.map(m => (
                    <th key={m.name} className="py-4 px-6 text-center text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest min-w-[120px] capitalize">
                      {m.name}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                <tr className="hover:bg-slate-50/30 transition-colors group">
                  <td className="py-5 px-6 font-bold text-slate-900 dark:text-slate-100 sticky left-0 bg-white dark:bg-slate-900 group-hover:bg-slate-50 transition-colors z-10 border-r border-slate-100 dark:border-slate-800">
                ...
                <Tooltip 
                  ...
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-800 animate-in fade-in zoom-in duration-300 min-w-[160px]">
                          <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 mb-1 uppercase tracking-widest capitalize">{data.name}</p>
                          <p className="text-lg font-black text-slate-900 dark:text-slate-100 mb-2">{formatCurrency(data.amount)}</p>
          ...
          <div className="mt-8 p-6 bg-slate-900 rounded-3xl text-white flex flex-col md:flex-row justify-between items-center gap-6 shadow-xl shadow-slate-200">
            <div>
              <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Total do Ano ({selectedYear})</p>
              <h4 className="text-4xl font-black text-white">
                {formatCurrency(historyMontlyData.reduce((acc, m) => acc + m.total, 0))}
              </h4>
            </div>
            <div className="text-right">
              <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1 text-slate-400 dark:text-slate-500">Média Mensal (Meses Concluídos)</p>
      ...
      <Card className="w-full bg-slate-900 text-white border-none overflow-hidden relative">
        ...
        <CardHeader className="relative z-10">
          ...
          <p className="text-slate-400 dark:text-slate-500">Visão consolidada de todas as contas.</p>
        </CardHeader>
        <CardContent className="relative z-10">
          <div className="grid md:grid-cols-3 gap-8">
            <div className="space-y-1">
              <p className="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-widest font-bold">Investido Total</p>
              <p className="text-3xl font-black text-white">{formatCurrency(metrics.totalInvested)}</p>
            </div>
            <div className="space-y-1">
              <p className="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-widest font-bold">Patrimônio Atual</p>
              <p className="text-3xl font-black text-emerald-400">{formatCurrency(metrics.totalMarketValue + metrics.totalDividends)}</p>
            </div>
            <div className="space-y-1">
              <p className="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-widest font-bold">Retorno Total acumulado</p>
          ...
          <div className="mt-8 pt-6 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <a href="/investments" className="text-xs font-bold bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 px-6 py-2.5 rounded-full hover:bg-slate-100 transition-colors">
    ...
    <Card className="hover:scale-[1.02] transition-all duration-300">
      <CardContent className="pt-6">
        ...
        <div className="mt-4">
          ...
          <p className="text-2xl font-bold text-slate-900 dark:text-slate-100 mt-1">{formatCurrency(value)}</p>
          {subtitle && <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1 font-medium">{subtitle}</p>}
        </div>
      </CardContent>
    </Card>
  );
